...
function run_passes(
    ...
    @pass "CC: ADCE"      ir = adce_pass!(ir, sv.inlining)
    @pass "CC: TYPE LIFT" ir = type_lift_pass!(ir)
    @pass "CC: COMPACT 3" ir = compact!(ir)
    if JLOptions().debug_level == 2
        @zone "CC: VERIFY 3" (verify_ir(ir); verify_linetable(ir.linetable))
    end
...
function slot2reg(ir::IRCode, ci::CodeInfo, sv::OptimizationState)
    ...
    @zone "CC: DOMTREE 1" domtree = construct_domtree(ir.cfg.blocks)
    defuse_insts = scan_slot_def_use(nargs, ci, ir.stmts.inst)
    𝕃ₒ = optimizer_lattice(sv.inlining.interp)
    @zone "CC: CONSTRUCT_SSA" ir = construct_ssa!(ci, ir, domtree, defuse_insts, sv.slottypes, 𝕃ₒ) # consumes `ir`
