...
import ..@noinline, ..Cint, ..Vector, ..push!, ..unsafe_convert, ..esc, ..pointer_from_objref
...
TracySrcLoc(zone_name::String, function_name::Symbol, file::Symbol, line::Integer, color::Integer) =
...
function _zone_create(name::String, ex::Expr, linfo::LineNumberNode)
    ...
    loc = TracySrcLoc(name, unknown_function, linfo.file, linfo.line, 0)
    push!(srclocs, loc)
    return loc
end
